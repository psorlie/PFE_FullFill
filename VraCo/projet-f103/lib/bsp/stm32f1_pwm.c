...
void PWM_run(timer_id_e timer_id, uint16_t TIM_CHANNEL_x, bool_e negative_chanel, uint32_t period_us, uint32_t duty, bool_e remap)
{
	...
	PWM_configure_pin(timer_id, TIM_CHANNEL_x, negative_chanel, TRUE, remap);
	...
	if(handler != NULL)
	{
		...
		PWM_set_duty(timer_id, TIM_CHANNEL_x, (uint16_t) duty);
	}
}

/** @brief	fonction qui règle le rapport cyclique sur le canal demandé.
 * @func	void PWM_set_duty(timer_id_e timer_id, uint16_t TIM_CHANNEL_x, uint16_t duty)
 * @param	timer_id : TIMERX_ID, où X est le numéro du timer
 * @param	TIM_CHANNEL_x, où x est le numéro du canal
...
void PWM_set_duty(timer_id_e timer_id, uint16_t TIM_CHANNEL_x, uint16_t duty)
{
	...
	duty = (uint16_t) ((duty*handler->Init.Period)/100);


	__HAL_TIM_SET_COMPARE(handler, TIM_CHANNEL_x, (uint32_t)duty);
}
