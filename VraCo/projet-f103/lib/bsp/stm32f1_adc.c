			...
			if(!t)
			{
				...
				for(channel = 0; channel < ADC_CHANNEL_NB; channel++)
				{
					if(adc_id[channel] != -1)
					{
						...
						millivolt = (((int32_t)value)*3000)/4096;									//On la convertie en volts
...
/** @brief 	fonction devant être appelée chaque milliseconde (par la routine d'interruption du timer par exemple)
...
static void PORTS_adc_init(void)
{
	...
	sConfig.Rank = 0;
		...
		adc_id[ADC_15] = sConfig.Rank;
		sConfig.Rank++;
		sConfig.Channel = ADC_CHANNEL_15;
		HAL_ADC_ConfigChannel(&hadc,&sConfig);
	#endif
	#if	USE_AN16
		adc_id[ADC_16] = sConfig.Rank;
		sConfig.Rank++;
		sConfig.Channel = ADC_CHANNEL_16;
		HAL_ADC_ConfigChannel(&hadc,&sConfig);
	#endif
	#if	USE_AN17
		adc_id[ADC_17] = sConfig.Rank;
		sConfig.Rank++;
...
/*	// Déclenchements de l'ADC par production d'un signal PWM (output compare du timer 1...)
	TIM_HandleTypeDef * htim;
	TIM_MasterConfigTypeDef sMasterConfig;
	TIMER_run_us(TIMER1_ID,1000);			//1 mesure par ms. (choix arbitraire, c'est un exemple...)
	htim = TIMER_get_phandler(TIMER1_ID);
	...
	HAL_TIM_OC_ConfigChannel(htim, &TIM_OCInitStructure, TIM_CHANNEL_1); //on configure le canal (avant on autorisait le prechargement de la config)
	...
	*/
	...
	TIM_HandleTypeDef * htim;
	TIM_MasterConfigTypeDef sMasterConfig;
	TIMER_run_us(TIMER3_ID,1000, FALSE);			//1 mesure par ms. (choix arbitraire, c'est un exemple...)
	...
	//HAL_ADCEx_MultiModeStart_DMA(&hadc,adc_converted_value,ADC_NB_OF_CHANNEL_USED);
