...
TODO :	BROKEN_A : le dispenser est cassé, on sort de la MAE
		TEXT_UPDATE_A : l'ack comprend des mise à jour de paramètres
		TEXT_EMPTY_A : pas de mise à jour de nouveaux paramètres
		CURFEW_BEGINNING_A : couvre feu fermeture magasin
		CURFEW_END_A ; fin couvre feun ouverture magasin
		SEND_OLD_VAL_A : envoi de la dernière mesure avant fermeture magasin
		RESTART_RTC_PERIODIC_ALARM_A : re-démarrage rtc pour délai entre 2 mesures
		RTC_PERIODIC_ALARM_A : alarme de la rtc pour nouvelle mesure

		Pour plus de précisions, voir le dossier conception
**/
static void observer_performAction(ObserverAction);

/**On vérifie le nombre de messages envoyés sans ack reçu**/
static void observer_check_nb_message_send();

/**l'alarm rtc se déclenche, on demande une mesure du capteur**/
...
/**TODO : on récupère le nouveau param dans le msg et modifie la configuration du dispenser**/
static void observer_update_param(char*);

/**MAE lien entre Etat et Action**/
static void* Observer_run();

/**Vérifie si la mesure est la même que précédement ou non**/
